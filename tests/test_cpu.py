...
def test_shift_out_sends_lsb_first():
    ...
    assert cpu.run() == [1, 1, 0, 1, 0, 0, 0]  # first 1 is the PULL cycle (pin idle high)


def test_shift_out_delay_holds_the_bit():
    assert CPU(assemble("PULL\nSHIFT_OUT [2]\nSHIFT_OUT"), tx_data=[0b01]).run() == [1, 1, 1, 1, 0]
...
def test_shift_out_order_pin_then_shift_on_first_cycle():
    ...
    cpu = CPU(assemble("PULL\nSHIFT_OUT [1]\nSHIFT_OUT"), pin=0, tx_data=[0b11])
    cpu.step()
    assert (cpu.pin, cpu.shift_reg) == (0, 0b11)  # PULL: register filled, pin untouched
    cpu.step()
