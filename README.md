...
| six of eight opcodes used before `PUSH` | the ISA itself | in progress: `SHIFT_OUT` + `SHIFT_IN` are one `SHIFT` opcode with an in/out bit, `CONFIG_SHIFT` became `CONFIG field, value`; a universal GPIO side effect and a `PULL`/`PUSH` bit are next |
